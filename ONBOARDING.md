...
| 8 | 会员等级特权 ⭐额外功能 | `displayTierPrivileges()` |
...
**recommendBooksForMember() ⭐额外功能（菜单选项 9）** — 个性化图书推荐：

```
读会员ID ──► 扫描 memberLoanHistory[mi][0..5] 找出借阅最多的类别（最喜爱类别）
        ├─ 有历史 → 筛出该类别的在架书目，按全馆 borrowCount 人气排序取前5
        └─ 无历史 → 回退：直接推荐全馆借阅次数最高的前5本
```

三步结构：找类别（扫 2D 数组一行）→ 收集候选（过滤 !active 和类别不符）→
选择法排序输出（和 M4 人气榜同款套路）。这功能把 M3 写入的数据变成了
看得见的价值——"数据在模块间流动"的最好例证，VIVA 讲额外功能就讲它。
...
| 13 | `1` → `8` | 会员等级特权表：三级别借书上限 3/4/6、罚款倍率 1.00/0.50/0.00（⭐额外功能1） |
| 14 | `2` → `9`，会员 `1001` | 推荐 Fiction 类：Silent Patient 排第1；换 `1005`（无历史）回退全馆人气榜（⭐额外功能2） |
